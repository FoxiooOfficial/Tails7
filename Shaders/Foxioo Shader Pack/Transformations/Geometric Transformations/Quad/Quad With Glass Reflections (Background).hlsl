...
PS_OUTPUT ps_main( in PS_INPUT In )
{
    ...
    _Render_Texture.a *= _In.z;
        ...
        _Render_Texture.rgb += (((S2D_Image.Sample(S2D_ImageSampler, _In.xy / _Luminance / 10.0)) * In.Tint).rgb + _Render_Background.rgb / 3.0) * _Mixing * (1 - __In.y) * _Luminance;
...
PS_OUTPUT ps_main_pm( in PS_INPUT In )
{
    ...
    _Render_Texture.a *= _In.z;
        ...
        _Render_Texture.rgb = lerp(_Render_Texture.rgb, _Render_Background.rgb * _Render_Texture.rgb, _Mixing);
        _Render_Texture.rgb += ((Demultiply(S2D_Image.Sample(S2D_ImageSampler, _In.xy / _Luminance / 10.0)) * In.Tint).rgb + _Render_Background.rgb / 3.0) * _Mixing * (1 - __In.y) * _Luminance;
